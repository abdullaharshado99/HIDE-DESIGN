...
async function seed() {
  ...
  for (const [articleNumber, name, audience, material, imageUrl] of catalogue) {
    ...
    if (!exists) await products.save(products.create({ articleNumber, name, audience, material, imageUrl, category: 'coats', description: material, price: null, currency: 'USD', published: true }));
  }
